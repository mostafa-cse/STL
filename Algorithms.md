1. **`<iostream>` (Console I/O)**
   ...
   1. `std::cout` / `std::cin` / `std::cerr` / `std::clog`
   2. `std::getline(cin, string)`
...
4. **`<cctype>` (Character Classification & Transformation)**
...
5. **`<cstring>` / `<cstdlib>` (C‐String Manipulation & Memory Routines)**

   1. C‐String (Null‐Terminated) Functions

      * `std::strlen(s)`
      * `std::strcpy(dst, src)` / `std::strncpy(dst, src, n)`
      * `std::strcat(dst, src)` / `std::strncat(dst, src, n)`
      * `std::strcmp(s1, s2)` / `std::strncmp(s1, s2, n)`
      * `std::strchr(s, c)` / `std::strrchr(s, c)`
      * `std::strstr(haystack, needle)`
   2. Memory Block Functions

      * `std::memset(ptr, val, n)`
      * `std::memcpy(dest, src, n)`
      * `std::memmove(dest, src, n)`
   3. Dynamic Allocation (C‐Style)

      * `std::malloc(n)`
      * `std::calloc(count, size)`
      * `std::realloc(ptr, newSize)`
      * `std::free(ptr)`
   4. Conversion & Process Control

      * `std::atoi(str)` / `std::atof(str)`
      * `std::exit(code)`
      * (`std::rand()`, `std::srand(seed)` – if used)

6. **`<utility>` (General Utilities)**

   1. `std::swap(a, b)`
   2. `std::pair<T1, T2>`, `std::make_pair(a, b)`
   3. `std::tie(...)`
   4. `std::exchange(obj, newValue)`

7. **`<algorithm>` (Generic Algorithms)**

   1. Mutators / Reordering

      * `std::sort(begin, end)`
      * `std::reverse(begin, end)` / `std::reverse_copy(begin, end, dest)`
      * `std::shuffle(begin, end, rand_engine)`
      * `std::rotate(begin, middle, end)`
      * `std::next_permutation(begin, end)`
      * `std::prev_permutation(begin, end)`
      * `std::partition(begin, end, pred)` / `std::stable_partition(begin, end, pred)`
      * `std::remove(begin, end, value)` / `std::remove_if(begin, end, pred)`  +  erase idiom
      * `std::replace(begin, end, oldVal, newVal)` / `std::replace_if(begin, end, pred, newVal)`
      * `std::unique(begin, end)` / `std::unique_copy(begin, end, dest)`
      * `std::fill(begin, end, value)` / `std::fill_n(begin, n, value)`
      * `std::generate(begin, end, genFunc)`
   2. Queries / Inspection

      * `std::find(begin, end, value)` / `std::find_if(begin, end, pred)`
      * `std::count(begin, end, value)` / `std::count_if(begin, end, pred)`
      * `std::min_element(begin, end)` / `std::max_element(begin, end)`
      * `std::minmax_element(begin, end)`
      * `std::binary_search(begin, end, value)` (requires sorted)
      * `std::lower_bound(begin, end, value)` / `std::upper_bound(begin, end, value)` (requires sorted)
      * `std::equal(begin1, end1, begin2)`
      * `std::mismatch(begin1, end1, begin2)`
      * `std::distance(begin, it)`
   3. Copy / Merge / Partition
   ...
   4. Partial Sorting / Selection

      * `std::nth_element(begin, nth, end)`

8. **`<numeric>` (Numeric Algorithms)**

   1. `std::iota(begin, end, value)`
   2. `std::accumulate(begin, end, init)`
   3. `std::partial_sum(begin, end, dest)`
   4. `std::adjacent_difference(begin, end, dest)`
   5. `std::inner_product(begin1, end1, begin2, init)`
   6. `std::gcd(a, b)` (C++17+)
   7. `std::lcm(a, b)` (C++17+)
   8. `std::reduce(begin, end, init, policy)` (C++17+, for parallel/unsequenced)

9. **`<limits>` / `<climits>` / `<cfloat>` (Numeric Limits & Macros)**
   ...
   1. **`<limits>` (type-safe)**
      ...
      * Boolean flags: `.is_signed`, `.is_integer`
   2. **`<climits>` (C macros for integer bounds)**
      ...
      * `LLONG_MIN`, `LLONG_MAX`
      * `ULLONG_MAX`
   3. **`<cfloat>` (C macros for floating-point bounds)**
...
10. **`<chrono>` / `<ctime>` (Time & Date)**
    ...
    2. **`<ctime>` (C-style)**
       ...
       * `std::time(nullptr)`
       * `std::localtime(&time_t)` / `std::gmtime(&time_t)`
       * `std::strftime(buffer, size, format, tm*)`
       * `std::difftime(time2, time1)`

11. **Containers & Their Member Functions**
    ...
    1. **`std::vector<T>`**
       ...
       * `.size()` / `.empty()`
       * `.clear()`
       * `.insert(it, elem)` / `.erase(it)`
       * `.reserve(n)` / `.capacity()`
       * Iterators: `.begin()`, `.end()`
       * Element access: `.front()`, `.back()`, `v[i]`, `v.at(i)`
    2. **`std::string`**
       ...
       * `.replace(pos, len, str)`
       * `.c_str()`
       * Conversions: `std::to_string(x)`, `std::stoi(str)`, `std::stod(str)`
    3. **`std::map<Key, Value>` / `std::unordered_map<Key,Value>`**
    ...
    4. **`std::set<T>` / `std::unordered_set<T>`**
       ...
       * `.erase(value)` / `.erase(it)`
       * Iterators: `.begin()`, `.end()`
    5. **`std::queue<T>` / `std::stack<T>` / `std::deque<T>` / `std::list<T>`**

       * **`std::queue<T>`:** `.push(val)`, `.pop()`, `.front()`, `.back()`, `.size()`, `.empty()`
       * **`std::stack<T>`:** `.push(val)`, `.pop()`, `.top()`, `.size()`, `.empty()`
       * **`std::deque<T>` / `std::list<T>`**: container-specific member functions (e.g., `.push_back`, `.push_front`, `.pop_back`, `.pop_front`, `.insert`, `.erase`, `.begin`, `.end`)
    6. **`std::array<T, N>`**
...
12. **Miscellaneous Utilities**
    ...
    1. **`<random>` (Random Number Generation)**
       ...
       * `std::random_device`
       * `std::mt19937`
       * Distributions: `std::uniform_int_distribution`, `std::uniform_real_distribution`, etc.
       * `std::shuffle(begin, end, engine)`
    2. **Additional Containers (briefly mentioned)**

       * `std::deque<T>` / `std::list<T>`
       * STL iterators & iterator adapters
    3. **Single Operators (not in `<…>`)**

       * `new` / `delete`
       * `new[]` / `delete[]`

13. **Header‐by‐Header Quick Reference**

    1. `<iostream>`: `std::cout`, `std::cin`, `std::cerr`, `std::clog`, `std::getline`
    2. `<fstream>`: `std::ifstream`, `std::ofstream`, `std::fstream`, `std::getline`
    3. `<string>`: `std::string`, `.length()`, `.substr()`, `.find()`, `std::to_string`, `std::stoi`, `std::stod`
    4. `<cstring>`: `std::strlen`, `std::strcpy`, `std::strcat`, `std::strcmp`, `std::strstr`, `std::memset`, `std::memcpy`, `std::memmove`
    5. `<cstdlib>`: `std::malloc`, `std::calloc`, `std::realloc`, `std::free`, `std::atoi`, `std::atof`, `std::exit`, `std::rand`, `std::srand`
    6. `<cmath>`: `std::abs`, `std::ceil`, `std::floor`, `std::round`, `std::sqrt`, `std::pow`, `std::exp`, `std::log`, `std::sin`, `std::cos`, etc.
    7. `<cctype>`: `std::isalpha`, `std::isdigit`, `std::isspace`, `std::toupper`, `std::tolower`
    8. `<algorithm>`: `std::sort`, `std::reverse`, `std::shuffle`, `std::rotate`, `std::next_permutation`, `std::partition`, `std::remove`, `std::replace`, `std::unique`, `std::fill`, `std::generate`, `std::find`, `std::count`, `std::min_element`, `std::binary_search`, `std::copy`, `std::merge`, `std::nth_element`, `std::distance`, etc.
    9. `<numeric>`: `std::accumulate`, `std::iota`, `std::partial_sum`, `std::adjacent_difference`, `std::inner_product`, `std::gcd`, `std::lcm`, `std::reduce`
    10. `<limits>`: `std::numeric_limits<T>::min()/max()/lowest()/epsilon()/infinity()/quiet_NaN()/denorm_min()`, `.is_signed`, `.is_integer`
    11. `<climits>`: `CHAR_BIT`, `SCHAR_MIN`, `SCHAR_MAX`, `UCHAR_MAX`, `SHRT_MIN`, `SHRT_MAX`, `USHRT_MAX`, `INT_MIN`, `INT_MAX`, `UINT_MAX`, `LONG_MIN`, `LONG_MAX`, `ULONG_MAX`, `LLONG_MIN`, `LLONG_MAX`, `ULLONG_MAX`
    12. `<cfloat>`: `FLT_MIN`, `FLT_MAX`, `DBL_MIN`, `DBL_MAX`, `FLT_EPSILON`, `DBL_EPSILON`
    13. `<chrono>`: `std::chrono::high_resolution_clock::now()`, `std::chrono::system_clock::now()`, `std::chrono::steady_clock::now()`, `std::chrono::duration_cast<…>`
    14. `<ctime>`: `std::time(nullptr)`, `std::localtime`, `std::gmtime`, `std::strftime`, `std::difftime`
    15. `<utility>`: `std::swap`, `std::pair`, `std::make_pair`, `std::tie`, `std::exchange`
    16. `<vector>`: `.push_back()`, `.emplace_back()`, `.pop_back()`, `.size()`, `.empty()`, `.clear()`, `.insert()`, `.erase()`, `.reserve()`, `.capacity()`, `.begin()`, `.end()`, `.front()`, `.back()`, `operator[]`, `.at()`
    17. `<array>`: `.fill()`, `.at()`, `.size()`, `.begin()`, `.end()`
    18. `<bitset>`: `.set()`, `.reset()`, `.flip()`, `.count()`, `.to_ulong()`, `.test()`
    19. `<map>` / `<set>`: `.insert()`, `operator[]`, `.find()`, `.erase()`, `.count()`, `.begin()`, `.end()`
    20. `<queue>` / `<stack>`: `.push()`, `.pop()`, `.front()`, `.back()`, `.top()`, `.size()`, `.empty()`
    21. `<random>`: `std::random_device`, `std::mt19937`, `std::uniform_int_distribution`, `std::shuffle`, etc.
